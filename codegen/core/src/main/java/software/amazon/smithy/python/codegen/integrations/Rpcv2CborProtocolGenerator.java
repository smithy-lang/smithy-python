...
public final class Rpcv2CborProtocolGenerator implements ProtocolGenerator {
    ...
    public void initializeProtocol(GenerationContext context, PythonWriter writer) {
        ...
        Symbol serviceSymbol = context.symbolProvider().toSymbol(context.settings().service(context.model()));
        SymbolReference serviceSchema = serviceSymbol.expectProperty(SymbolProperties.SCHEMA);
        writer.write("$1T($2T)", RuntimeTypes.RPC_V2_CBOR_CLIENT_PROTOCOL, serviceSchema);
    }
