...
from aws_event_stream.auth import Credentials, EventSigner
from aws_event_stream.eventstream import (
...
class IdentityParser(EventParser):
    def parse(self, event: EventStreamMessage) -> Any:
        return event


def assert_message_equal(message_a: EventStreamMessage, message_b: EventStreamMessage):
...
def check_message_decodes(encoded: bytes, decoded: EventStreamMessage):
...
def test_positive_cases(encoded: bytes, decoded: EventStreamMessage):
...
def test_negative_cases(encoded: bytes, exception: type[Exception]):
    """Test that all negative cases raise the expected exception."""
    with pytest.raises(exception):
        check_message_decodes(encoded, None)  # type: ignore
...
def create_mock_raw_stream(*data: bytes):
    raw_stream = Mock()
...
async def test_event_stream_wrapper_iteration():
    ...
    events: list[EventStreamMessage] = []
...
SERIALIZATION_CASES: list[tuple[bytes, dict[str, Any], bytes]] = [
...
class TestEventStreamMessageSerializer:
    ...
    def test_serialized_message(
        self,
        serializer: EventStreamMessageSerializer,
        expected: bytes,
    ...
    def test_invalid_header_value(self, serializer: EventStreamMessageSerializer):
        ...
        with pytest.raises(InvalidHeaderValue):
            serializer.serialize(headers, b"")  # type: ignore
    ...
    def test_header_bytes_too_long(self, serializer: EventStreamMessageSerializer):
    ...
    def test_headers_too_long(self, serializer: EventStreamMessageSerializer):
    ...
    def test_payload_too_long(self, serializer: EventStreamMessageSerializer):
...
class TestEventSigner:
    ...
    def utc_now(self):
        return datetime.datetime(2020, 7, 23, 22, 39, 55, 29943, tzinfo=datetime.UTC)

    def test_basic_event_signature(
        self, event_signer: EventSigner, credentials: Credentials
    ):
