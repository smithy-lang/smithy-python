...
Naive: the entire source is read into memory up front and a single cursor walks it.
Both definite- and indefinite-length aggregates are accepted on read (indefinite is
decoder-only tolerance; the serializer emits only definite forms). Numeric widening
(byte/short/long -> integer, double -> float) is inherited from the ``ShapeDeserializer``
base defaults.
"""
...
# The initial byte of a "break" stop code (major 7, ai 31), ending an indefinite item.
_BREAK = 0xFF
